...
| B4 | 偏差値表示に変更 | 高 | 表示ラベルを「スコア」→「偏差値」に変更。ScoreGauge・SafetySection・比較テーブル・ランキング等の全箇所。計算ロジックは既に偏差値ベース（平均50, σ=10）なのでUIのみの変更 | ⬜ |
| B5 | 町丁目ベース検索・ページ追加 | 中 | 駅ベース（659駅）に加え、町丁目単位でのデータ表示・検索を追加。犯罪CSVは元々町丁目単位のため、駅への集約前のデータを活用。Phase B: 町丁目テーブル+住所検索、Phase C: 町丁目ページSSG（数千ページ）。駅ページは維持しつつ、番地・町名からの検索を追加することで駅から遠い引越し検討者もカバーする | ⬜ |
